...
### 6.1 Missing HMAC Key for Inter-Module Auth

**Error**: `401 Unauthorized` in inter-module HTTP calls during tests

**Symptom**: Tests fail with authentication errors when one module calls another via HTTP (e.g., broker calling datafeed)

**Cause**: HMAC key not generated for inter-module request signing

**Solution**:

```bash
# Generate HMAC key (auto-generated by test targets)
make check-generate-hmac-key

# Verify key exists
ls -la backend/.local/secrets/hmac_internal.key
```

**CI Note**: The CI workflow generates both JWT and HMAC keys:

```yaml
- name: Generate keys for tests
  working-directory: backend
  run: |
    make check-generate-keypair
    make check-generate-hmac-key
```
