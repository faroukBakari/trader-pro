...
- **Datastore Implementation**: `MODULAR_BACKEND_ARCHITECTURE.md` (service datastore injection) → `datastores/README.md` (implementation guide, Feature Detection) → `datastore_interface.py` (abstract interface, has_persistence, has_transactions) → `inmemory_datastore.py` (in-memory implementation) → `datastores/postgres/datastore.py` (PostgreSQL + JSONB implementation)
