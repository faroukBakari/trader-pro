...
| **docs/methodologies/IA-COORDINATION-METHODOLOGY.md** | ⭐ Three-layer agentic design model (Agents, Prompts, Skills) |
...
| **"Bar caching/gap detection/read-through cache"** | **`modules/datafeed/README.md` (Cache Management Layer, Read-Through Cache Orchestration) → `bar_cache_manager.py` (BarCacheManager) → `service.py` (get_bars orchestration) → `models/market/bar_cache.py`**                                                                  | **Module doc first**   |
| **"Cache orchestration/read-through cache"**       | **`modules/datafeed/README.md` (Read-Through Cache Orchestration section) → `service.py` (get_bars, \_process_gap, \_fetch_and_cache_gap, \_wait_for_gap_coverage) → `test_api_integration.py::TestGetBarsCaching`**                                                           | **Module doc first**   |
...
| **"PostgresDatastore/SQLModelTable storage"**      | **`datastores/README.md` (PostgresDatastore section) → `datastores/postgres/datastore.py` (psycopg3 + SQLModelTable) → `datastore_interface.py`**                                                                                                                              | **Implementation doc** |
...
| **"Transaction batching/session injection"**       | **`datastores/README.md` (Transaction Support section) → `postgres/README.md` (Session Scope Pattern) → `datastore_interface.py` (session_factory property)**                                                                                                                  | **Implementation doc** |
| **"Atomic operations/mark_covered"**               | **`datafeed/README.md` (Cache Management Layer) → `bar_cache_manager.py` (mark_covered transaction) → `datastores/README.md` (Transaction Support)**                                                                                                                           | **Module doc first**   |
| **"Timeseries/time-range queries/has_timeseries"** | **`datastores/README.md` (Interface Segregation section, Feature Detection table) → `datastore_interface.py` (TimeSeriesTableInterface) → `repository.py` (has_timeseries capability check)**                                                                                  | **Interface doc**      |
| **"Range query/gap detection/internal gaps"**      | **`datastores/README.md` (Interface Segregation section, RangeQueryTableInterface) → `sqlmodel_table.py` (RangeQuerySQLModelTable) → `bar_cache_manager.py` (has_rangequery capability check)**                                                                                | **Interface doc**      |
| **"Capability-based datastore/selection"**         | **`datastores/README.md` (Capability-Based Selection section) → `datastore_interface.py` (capabilities() abstract method) → `service_interface.py` (datastore_capabilities(), \_resolve_datastore_capabilities()) → `module_registry.py` (required_datastore_capabilities())** | **Architecture first** |
| **"DatastoreCapabilitySpec/optional capability"**  | **`models/common.py` (DatastoreCapabilitySpec) → `datastores/README.md` (Capability Matching table) → `test_datastore_capability_selection.py` (12 tests)**                                                                                                                    | **Model doc first**    |
...
- **Datastore Implementation**: `MODULAR_BACKEND_ARCHITECTURE.md` (service datastore injection) → `datastores/README.md` (implementation guide, Feature Detection) → `datastore_interface.py` (abstract interface, has_capability(), capabilities()) → `inmemory_datastore.py` (in-memory implementation) → `datastores/postgres/datastore.py` (PostgreSQL implementation)
- **Capability-Based Datastore Selection**: `datastores/README.md` (Capability-Based Selection section) → `models/common.py` (DatastoreCapabilitySpec) → `datastore_interface.py` (capabilities() abstract method) → `service_interface.py` (datastore_capabilities(), \_resolve_datastore_capabilities()) → `module_registry.py` (required_datastore_capabilities()) → `datastore_registry.py` (get_datastores() with capability filtering)
- **Transaction Support**: `datastores/README.md` (Transaction Support section, session parameter) → `postgres/README.md` (Session Scope Pattern, ownership semantics) → `sqlmodel_table.py` (\_session_scope implementation) → `bar_cache_manager.py` (mark_covered atomicity example)
...
**Keywords**: .agent.md, custom agents, subagents, runSubagent, handoffs, agent orchestration, YAML frontmatter, user-invokable, disable-model-invocation, tool restrictions, agent composition, Claude Opus 4.6, Claude Sonnet 4.5, agent prompts, constraint hierarchy, XML sections, context isolation, parallel execution, three-layer model, boundary separation, quality gates, .prompt.md, prompt files, agent skills, AgentSkills.io, ia-coord, skill extraction

**Scope**: VS Code custom agent configuration, three-layer agentic design (Agents/Prompts/Skills), and Claude-optimized prompt engineering  
**Out of Scope**: GitHub Actions, CI bots, external automation

- **docs/methodologies/IA-COORDINATION-METHODOLOGY.md** - ⭐ Three-layer agentic design model: boundary separation, quality gates, model selection, skill extraction
- **.github/agents/ia-coord.agent.md** - Master IA coordinator (creates agents, prompts, skills with enforced boundaries)
...
**Keywords**: system design, component architecture, design patterns, module structure, provider system, broker integration, modular architecture, datastore abstraction, DatastoreInterface, TableInterface, TableInterface[T], TimeSeriesTableInterface, RangeQueryTableInterface, Generic[T], type-safe tables, RWLock, InMemoryDatastore, PostgresDatastore, DatastoreRegistry, auto-discovery, per-table locking, datastore injection, construction-time index configuration, indexes kwarg, unique_indexes kwarg, create_index, create_unique_index, auto-indexing, secondary index, unique index, async CRUD, get_all, iterate, values, AsyncIterator, ValueFactory, DataNotFoundError, datastore_name classmethod, has_capability(), DatastoreCapabilityName, capability names (persistence, transactions, timeseries, rangequery, exclusion), timeseries_table, rangequery_table, get_time_range, set_batch, get_missing_ranges, INT8MULTIRANGE, range_agg, multirange subtraction, psycopg, async factory, requires_async_init, register_factory, get_datastores_async, Range[T], TimeRange, PendingRange, CoveredRange, BarCacheManager, StorageType, gap detection, internal gap detection, read-through cache, pending range, covered range, types package, Comparable Protocol, boundary-based gap detection, BarCacheManager.create, lookup_key, model_post_init, async factory pattern, BAR_CACHE_PENDING_TTL_MS, dynamic table naming, bars_symbol_resolution, SQLModelTable typed returns, BarRepository, dynamic Bar subclass, IntRange, DateTimeRange, DateOnlyRange, Int8RangeType, TstzRangeType, DateRangeType, requires_gist_index, GiST index, exclusion constraint, EXCLUDE USING GIST, btree_gist, exclusion_listener, DatastoreException, session injection, session_factory, \_session_scope, ownership-based commit, transaction batching, atomic operations, auto-commit, caller-managed transaction, read your writes, Unit of Work propagation, PostgresTable removal, NoInspectionAvailable, table=True requirement, get_table_name raises, Field(primary_key=True) mandatory, create_dynamic_table_model, DatastoreCapabilitySpec, datastore_capabilities, provider_capabilities, ProviderCapabilitySpec, capability-based selection, required_datastore_capabilities, capability matching, optional capability
...
- **2026-02-07**: IA Coordination Methodology & Three-Layer Model - Replaced `AGENT-MD-METHODOLOGY.md` with `IA-COORDINATION-METHODOLOGY.md` documenting the three-layer agentic design model (Prompt=WHAT, Agent=HOW, Skill=HOW-TO) with boundary separation theory, 4 violation types, quality gates (A1-A9 agents, P1-P6 prompts, S1-S5 skills), model selection guide (Haiku 0.33x/Sonnet 1.0x/Opus 3.0x), skill extraction heuristics, and anti-pattern catalog. Created `ia-coord.agent.md` master coordinator with 3 templates (`agent-template.md`, `prompt-template.md`, `skill-template.md`). Deprecated legacy agents (`agent-gen`, `prompt-gen`, `skill`) in favor of unified `ia-coord`. Updated copilot-instructions.md Section 9 (agent catalog, quick decision rules). Original `.github/agents/` directory with 6 starter agents created earlier: `study.agent.md`, `plan.agent.md`, `implement.agent.md`, `review.agent.md`, `test.agent.md`, `research.agent.md`.
...
- **2026-02-04**: TimeSeriesTableInterface & has_timeseries Capability - Added `has_timeseries` property to `DatastoreInterface` (PostgresDatastore=True, InMemoryDatastore=False). Added `TimeSeriesTableInterface` extending `TableInterface` with `get_time_range()` and `set_batch()` methods. Refactored `BarRepository` to use `has_timeseries` capability check instead of try/except pattern. Simplified Bar model fields (removed `sa_column` overrides). Removed `persistent_datastore` property from `ServiceInterface`. Removed `is_relational` property from `DatastoreInterface`. Added `create_dynamic_table_model()` utility to `shared/__init__.py`. Added 6 PostgreSQL integration tests for `TimeSeriesTableInterface` (`test_repository.py::TestPostgresBarRepository`). Updated datastores/README.md (Feature Detection table with `has_timeseries`, Interface Segregation example with capability check pattern), datafeed/README.md (Repository Layer with `TimeSeriesTableInterface` usage), DOCUMENTATION-GUIDE.md (keywords: has_timeseries, TimeSeriesTableInterface, timeseries_table, get_time_range, set_batch, create_dynamic_table_model; query pattern: "Timeseries/time-range queries/has_timeseries"; timeline).
- **2026-02-03**: PostgresTable Removal & SQLModel Mandatory - Removed PostgresTable class (~520 lines of JSONB storage code), making SQLModelTable the sole storage implementation. Updated `get_table_name()` to raise `NoInspectionAvailable` instead of returning None. Removed `primary_key` parameter from `DatastoreInterface.table()` - primary key now MUST be declared via `Field(primary_key=True)`. Cleaned up exports: `InMemoryTable` and `SQLModelTable` no longer exported from top-level `datastores/__init__.py`. Updated datastores/README.md (JSONB references removed, PostgresDatastore section simplified to typed column storage only, DatastoreInterface.table() signature updated, architecture diagram simplified), postgres/README.md (Model Requirements section added, dual-mode language removed, Overview clarified), DOCUMENTATION-GUIDE.md (Architecture & Design keywords: removed JSONB, added NoInspectionAvailable/table=True requirement/get_table_name raises/Field(primary_key=True) mandatory; Query Pattern Mapping: PostgresDatastore/JSONB storage → PostgresDatastore/SQLModelTable storage; timeline).
- **2026-02-02**: Session Injection & Transaction Support (SQLModelTable) - Added `session` parameter to all `TableInterface` CRUD methods (`get`, `get_all`, `set`, `delete`, `exists`, `clear`) enabling transaction batching. Implemented `_session_scope()` context manager in `SQLModelTable` with ownership-based commit semantics (caller-provided session = caller commits, None = auto-commit). Added `session_factory` property to `DatastoreInterface` for callers to create transaction sessions. Updated `BarCacheManager.mark_covered()` to use atomic transaction (delete pending + insert covered). Added `has_transactions` validation check in `BarCacheManager.__init__`. Updated datastores/README.md (Transaction Support section, TableInterface Methods table with session param), postgres/README.md (Session Scope Pattern section with ownership matrix, read-your-writes, rollback examples), datafeed/README.md (Atomic Transitions feature, mark_covered() atomicity documentation), DOCUMENTATION-GUIDE.md (keywords: session injection, session_factory, \_session_scope, ownership-based commit, transaction batching, atomic operations, auto-commit, caller-managed transaction, read your writes, Unit of Work propagation; query patterns: "Transaction batching/session injection", "Atomic operations/mark_covered"; timeline).
- **2026-02-03**: BackendManager Integration Tests Refactoring - Refactored `test_backend_manager_integration.py` from single `TestBackendManagerIntegration` class to two-class design: `TestBackendManagerReadOnly` (module-scoped fixtures, servers never stopped) and `TestBackendManagerLifecycle` (function-scoped fixtures, fresh per test). Added `get_unique_port_base()` for deterministic port allocation via test name hashing (prevents parallel test collisions). Implemented emergency cleanup pattern with `_active_managers` dict and `atexit.register(_emergency_cleanup)` for orphaned manager termination. Changed `session_backend_manager` fixture scope from "session" to "module" for xdist compatibility. Added `@pytest.mark.xdist_group` markers. Updated `dev-config.yaml` to include `auth` module in broker server. Enhanced `AppFactory` startup log with module/provider/datastore lists. Updated BACKEND_TESTING.md (Section 4 two-class design pattern, module-scoped fixture rationale, emergency cleanup code, ensure_started guidance table), BACKEND_MANAGER_GUIDE.md (config example with auth module, integration test commands with class names), DOCUMENTATION-GUIDE.md (keywords: TestBackendManagerReadOnly, TestBackendManagerLifecycle, two-class test design, module-scoped fixtures, xdist_group, get_unique_port_base, emergency cleanup, atexit cleanup, \_active_managers, lifecycle manager fixture, port allocation, parallel test execution; timeline).
- **2026-02-02**: PostgreSQL Range Types & Exclusion Constraints (Wave 2C) - Added native PostgreSQL range types (`IntRange`, `DateTimeRange`, `DateOnlyRange`) with SQLAlchemy TypeDecorators (`Int8RangeType`, `TstzRangeType`, `DateRangeType`) that auto-create GiST indexes via `requires_gist_index` marker. Implemented declarative exclusion constraints via `__table_args__["info"]["exclusion"]` pattern with `exclusion_listener.py` creating EXCLUDE USING GIST constraints on table creation (auto-creates btree_gist extension for text column support). Added `has_exclusion` property to DatastoreInterface (InMemoryDatastore fails fast on exclusion models). Migrated `PendingRange`/`CoveredRange` to SQLModel tables with int8range columns. Added `DatastoreException` class for datastore layer errors. Updated datastores/README.md (Feature Detection table, removed deprecated index methods, Exclusion Constraints section), postgres/README.md (Directory Structure, Range Types section, Exclusion Constraints section), ERROR-MANAGEMENT.md (DatastoreException documentation), DOCUMENTATION-GUIDE.md (keywords: IntRange, DateTimeRange, DateOnlyRange, Int8RangeType, TstzRangeType, DateRangeType, requires_gist_index, GiST index, exclusion constraint, EXCLUDE USING GIST, btree_gist, exclusion_listener, has_exclusion, DatastoreException; timeline).
- **2026-01-31**: BarCacheManager & BarRepository DatastoreInterface Migration - Migrated `BarCacheManager` from in-memory dicts to `TableInterface` with async factory pattern (`create()` method) and settings injection (`BAR_CACHE_PENDING_TTL_MS`). Added `lookup_key` computed field to `PendingRange` and `CoveredRange` models via `model_post_init()` for indexed datastore lookups. Migrated `BarRepository` to create dynamic tables per symbol/resolution (`bars_{symbol}_{resolution}`) using dynamic `Bar` subclasses with `__tablename__`. Updated `PostgresTable` to return typed model instances instead of dicts via `model_validate()` (matching InMemoryTable behavior). Removed `BarRepositoryType` union alias. All `BarCacheManager` methods now async. Updated datafeed/README.md (Repository Layer with TableInterface pattern, Cache Management factory pattern, Models table with lookup_key), datastores/README.md (PostgresTable typed returns, test description updates), DOCUMENTATION-GUIDE.md (keywords: BarCacheManager.create, lookup_key, model_post_init, async factory pattern, BAR_CACHE_PENDING_TTL_MS, dynamic table naming, bars_symbol_resolution, PostgresTable typed returns; timeline).
- **2026-01-31**: Bar Cache Infrastructure - Added `BarCacheManager` class (`modules/datafeed/bar_cache_manager.py`) for read-through cache metadata tracking with gap detection algorithm. Created generic `Range[T]` type (`types/range.py`) with `Comparable` Protocol and TYPE_CHECKING conditional for Pydantic v2 compatibility. Added `StorageType` enum (`types/storage.py`) for cache tier tracking (MEMORY, DATABASE, DATALAKE). Created cache models (`models/market/bar_cache.py`): `TimeRange` (base range with int milliseconds), `PendingRange` (in-flight request with TTL expiration), `CoveredRange` (cached data with storage type). Added 24 unit tests (`test_bar_cache_manager.py`) covering pending/covered ranges, gap detection, cleanup. Updated `modules/datafeed/README.md` (Cache Management Layer section, Models table, test file documentation), `DOCUMENTATION-GUIDE.md` (keywords: Range[T], TimeRange, PendingRange, CoveredRange, BarCacheManager, StorageType, gap detection, read-through cache, pending range, covered range, types package, Comparable Protocol, boundary-based gap detection; query pattern: "Bar caching/gap detection/read-through cache"; timeline).
...
- **2026-01-27**: InMemoryDatastore Async Refactoring & Indexing - Refactored all CRUD operations to async (`set`, `get`, `get_all`, `delete`, `iterate`, `exists`, `clear`), added **secondary indexing** via `create_index(field)` (1:N field→records mapping with auto-indexing on `set()`), added **unique index constraint** via `create_unique_index(field)` (1:1 mapping with `ValueError` on duplicate), added `get_all()` method for bulk retrieval, renamed snapshot/restore to `export()`/`import_data()`, removed `TransactionContext` (unused complexity), added comprehensive test suite (`test_inmemory_datastore.py`: 23 tests covering CRUD, indexing, uniqueness, concurrency, edge cases), created `datastores/README.md` implementation documentation. Updated MODULAR_BACKEND_ARCHITECTURE.md (version 6.2.0, Provider System Integration diagram with datastores), DOCUMENTATION-GUIDE.md (keywords: create_index, create_unique_index, auto-indexing, secondary index, unique index, async CRUD, get_all, iterate; query patterns; Feature Implementation Chains: Datastore Implementation; timeline)
- **2026-01-26**: Datastore Abstraction Layer - Added `DatastoreInterface`, `TableInterface`, `RWLock`` interfaces (`shared/datastore_interface.py`), `InMemoryDatastore` implementation (`datastores/inmemory_datastore.py`) with per-table read-write locking, updated `ServiceInterface`to require`datastores`parameter and provide`self.datastore` property, removed abstract repository interfaces (`BarRepository`, `UserRepository`, `RefreshTokenRepository`), updated all concrete repositories to accept `DatastoreInterface`injection, changed`ModuleRegistry.get_modules()` signature (`None`→ empty list`[]`for all modules), removed`ModuleRegistry.get_module()`method, deleted`no_modules_app` test fixture. Updated MODULAR_BACKEND_ARCHITECTURE.md (service example with datastore injection, lazy loading example, version 6.1.0), AUTHENTICATION.md (repository layer datastore injection), BACKEND_TESTING.md (fixture notes), module READMEs (auth, datafeed, broker: datastore injection documentation), DOCUMENTATION-GUIDE.md (keywords: DatastoreInterface, TableInterface, RWLock, InMemoryDatastore, datastore injection, per-table locking; query patterns: "Datastore/persistence/storage", "Repository datastore injection", "RWLock/concurrent access"; timeline)
