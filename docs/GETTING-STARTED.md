...
# Verify authentication keys exist (auto-generated by make install)
ls backend/.local/secrets/
# Should show: jwt_private.pem, jwt_public.pem, hmac_internal.key
```

**Note**: Authentication keys (JWT keypair and HMAC key) are auto-generated by `make install` or test targets. If missing, regenerate with:

```bash
cd backend
make check-generate-keypair    # JWT keys for user auth
make check-generate-hmac-key   # HMAC key for inter-module auth
```
