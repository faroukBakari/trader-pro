...
6. **Phase 1: Create Application Factory** ✅ - Completed 2025-10-29
   ...
   - Created `app_factory.py` with `mount_app_modules()` function
...
21. **Phase 5: Refactor main.py to use app_factory pattern** ✅ - Completed 2025-10-29
    ...
    - Switched to mount_app_modules() factory pattern
...
- **Application Factory** (`app_factory.py`) - Dynamic app composition with `mount_app_modules()`
...
- **Pattern**: Module-specific conftest.py creates app via `mount_app_modules(enabled_modules=["module_name"])`
- **Implementation**:
  ```python
  ...
  def apps():
      from trading_api.app_factory import mount_app_modules
      return mount_app_modules(enabled_modules=["broker"])  # Only broker
  ```
...
- **Implementation**:
  ...
  def create_test_app(enabled_modules: list[str] | None = None):
      from trading_api.app_factory import mount_app_modules
      return mount_app_modules(enabled_modules=enabled_modules)
  ```
...
- **Rule**: All app instances created via `mount_app_modules()`, never import from `main.py`
- **Forbidden**: `from trading_api.main import apiApp` ❌
- **Allowed**: `from trading_api.app_factory import mount_app_modules` ✅
...
- **Rule**: `app_factory.mount_app_modules()` MUST clear registry before registering modules
- **Implementation**:
  ```python
...
```
backend/src/trading_api/
├── main.py                  # Minimal - calls mount_app_modules()
├── app_factory.py           # Application factory
...
def mount_app_modules(enabled_modules: List[str] | None = None) -> tuple[FastAPI, FastWSAdapter]:
    registry.register(DatafeedModule())
    registry.register(BrokerModule())
...
1. **Application Factory** - Dynamic module composition via `mount_app_modules()`
...
# def _get_current_app():
#     from trading_api.app_factory import mount_app_modules
#     api_app, _ = mount_app_modules(enabled_modules=None)
#     return api_app
...
2. **Create module-specific conftest** (example: `modules/broker/tests/conftest.py`):
   ...
   def apps():
       ...
       from trading_api.app_factory import mount_app_modules
       return mount_app_modules(enabled_modules=["broker"])
...
- ✅ **Factory pattern only**: All apps created via `mount_app_modules()`, never import from `main.py`
...
2. **Create integration test conftest** (`backend/tests/integration/conftest.py`):
   ...
   def apps():
       ...
       from trading_api.app_factory import mount_app_modules
       return mount_app_modules(enabled_modules=None)  # None = all modules
...
5. **Create module isolation validation tests** (`backend/tests/integration/test_module_isolation.py`):
   ...
   def test_datafeed_only_isolation():
       ...
       api_app, _ = mount_app_modules(enabled_modules=["datafeed"])
   ...
   def test_broker_only_isolation():
       ...
       api_app, _ = mount_app_modules(enabled_modules=["broker"])
       openapi_spec = api_app.openapi()
       paths = openapi_spec.get("paths", {})
...
**Objective**: Remove global service instances from `main.py` and switch to using `mount_app_modules()` from `app_factory.py`. This completes the factory pattern migration and eliminates the last remaining monolithic code.
...
- `main.py` calls `mount_app_modules()` to get configured application
...
- Switch to mount_app_modules() factory pattern
...
@pytest.fixture
def datafeed_only_app():
    api_app, ws_app = mount_app_modules(enabled_modules=["datafeed"])
    return api_app
...
│  - Calls mount_app_modules(enabled_modules)                                │
