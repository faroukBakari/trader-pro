...
description: Reasoning guardrails preventing design-spiral in implementation. Load when planning implementation or detecting drift
keywords: [reasoning, implementation, guardrails, materialization, design-spiral]
category: reasoning
...
> **Why this matters**: Benchmark data (Feb 2026) shows first-attempt success drops from ~70-80% on simple tasks to ~25-40% on compound plans. Verification at each step is not overhead — it's the difference between 40% and 80% success. See `plan-implement` skill § "Plan Execution Model" for the full odds table and checkpoint protocol.
