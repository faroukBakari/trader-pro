...
keywords: [introspection, retrospective, session-analysis, tuning, continuous-improvement, optimization]
category: review
...
| **IA Stack** | `.claude/CLAUDE.md`, `.claude/skill-bank/`, `.claude/skills/` | Current rules, skills, routing — the thing being tuned |
...
| Signal | Search Strategy | What to Look For |
|--------|----------------|------------------|
| Skill loaded | Search for `Read .claude/skill-bank/` | Which skills were actually loaded |
| Skill should have loaded | Cross-reference task type with category glossaries | Task matches a skill's trigger but skill wasn't loaded |
...
| `glossary-rebuild` | Regenerate after keyword/category changes | Always follows skill-keyword changes |
...
- Group related adjustments (e.g., keyword change + glossary rebuild)
...
|----|------|--------|--------|------|--------|----------|
| ADJ-...-01 | skill-keyword | `.claude/skill-bank/X/SKILL.md` | Add keyword `Y` | high | trivial | 4.0 |
...
2. For `skill-*` changes → follow `ia-stack-ops` Workflow A
3. For `rule-*` / `routing-*` changes → follow `ia-stack-ops` Workflow C
4. For `permission-tune` → edit `.claude/settings.json` directly
5. For `memory-note` → update auto-memory files
6. Run `build-skill-tree.py --write` if any skill frontmatter changed
7. Record applied adjustments in auto-memory for tracking over time
