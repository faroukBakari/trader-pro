---
name: terminal-usage
description: Terminal command safety checks and delegation routing. Load when running shell commands or delegating execution
keywords: [terminal, commands, safety-checks, delegation, execution]
category: workflow
...
Delegate via `Task(subagent_type="Bash")` with the `command-execution` skill injected in the prompt. The skill provides command guards (G1-G5) for auto-approved, safe execution.
