...
poetry run python -c "from trading_api.app_factory import mount_app_modules; mount_app_modules()"
# ✅ Output:
...
```
mount_app_modules() execution with per-module generation:
├─ 1. registry.clear()
...
1. `mount_app_modules()` loads each enabled module
