...
class TWSBrokerProvider(Provider, BrokerCapability):
    ...
    async def place_order(self, order: PreOrder) -> PlaceOrderResult:
        ...
        # Submit order via TWSClient (allocates order ID internally)
        order_id = self._tws_client.placeOrder(contract, tws_order)
    ...
    async def modify_order(self, order_id: str, order: PreOrder) -> None:
        ...
        # Re-submit with same order ID to modify
        self._tws_client.modifyOrder(int(order_id), contract, tws_order)
    ...
    async def close_position(
        ...
        # Place the closing order (order ID allocated internally)
        self._tws_client.placeOrder(contract, order)
    ...
    async def get_leverage_info(self, params: LeverageInfoParams) -> LeverageInfo:
        ...
        # Use TWSClient's placeWhatIfOrder which handles one-shot callback internally
    ...
    async def _on_tws_order_update(self, tracked: TrackedOrder) -> None:
        """Internal handler for TWS order updates - dispatches to all subscribers."""
        placed_order = tracked_order_to_placed_order(tracked)
