...
def createDeltaNeutralContractProto(contract: Contract) -> DeltaNeutralContractProto:
    assert contract.deltaNeutralContract is not None, "DeltaNeutralContract is None"
...
def createOrderProto(order: Order) -> OrderProto:
    ...
    if isValidFloatValue(order.discretionaryAmt):
...
def createOperatorConditionProto(
    ...
    if operatorCondition.isMore is not None:
...
def createContractConditionProto(
    ...
    if contractCondition.conId is not None and isValidIntValue(contractCondition.conId):
...
def createPriceConditionProto(priceCondition: PriceCondition) -> OrderConditionProto:
    ...
    if priceCondition.price is not None and isValidFloatValue(priceCondition.price):
        priceConditionProto.price = priceCondition.price
    if priceCondition.triggerMethod is not None and isValidIntValue(
        priceCondition.triggerMethod
    ):
...
def createVolumeConditionProto(volumeCondition: VolumeCondition) -> OrderConditionProto:
    ...
    if volumeCondition.volume is not None and isValidIntValue(volumeCondition.volume):
...
def createSoftDollarTierProto(order: Order) -> SoftDollarTierProto:
    ...
    assert tier is not None, "SoftDollarTier is None"
